...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const isStdin = argv[2] === '-' || argv[2] === 'stdin';
  const inputPath = isStdin ? null : path.resolve(argv[2] || path.join(skillRoot, 'examples', defaultExample));
  const rawData = isStdin ? fs.readFileSync(0, 'utf8') : fs.readFileSync(inputPath, 'utf8');
  const diagram = JSON.parse(rawData);
  ...
  const sourceEvidence = isStdin ? null : verifyRepositoryEvidence(diagramType, diagram, process.env.ARCHIFY_REPO_ROOT);
