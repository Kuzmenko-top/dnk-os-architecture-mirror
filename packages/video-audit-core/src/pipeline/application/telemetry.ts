/*
# --- DNK-MRH-HEADER ---
# mrh_id: "packages/video-audit-core/src/pipeline/application/telemetry.ts"
# purpose: "Application-level LLM execution telemetry, dashboard metrics, and local JSON logger for video-audit-core."
...
export class MockLangfuseTelemetry implements LlmTelemetry {
  public traces: Array<{ id: string; name: string; metadata: any }> = [];
  public generations: Array<{ model: string; usage: any; latency: number; metadata: any }> = [];
  private readonly fallback: LocalJsonTelemetry;

  constructor(fallback?: LocalJsonTelemetry) {
    this.fallback = fallback ?? new LocalJsonTelemetry();
  }
  ...
  public async recordMetrics(metrics: LlmMetrics): Promise<void> {
    ...
    this.generations.push({
      ...
      latency: metrics.latencyMs / 1000,
      metadata: {
  ...
  public getFallback(): LocalJsonTelemetry {
